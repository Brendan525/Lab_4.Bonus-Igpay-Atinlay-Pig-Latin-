    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            static void Consonants(ref string word)
            {
                ...
                {
                    Regex entry = new Regex(@"^([bcdfghjklmnpqrstvwxyz]([^0-9|@|!|#|$|%|^|&|*|(|)|-|=|+|_|[|\]|\|\||,|<|.|>|/|\|?]))"); // Regex entry = new Regex(@"^([bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ])");              
            ...
            static void Vowels(ref string word)
            {
                ...
                Regex entry = new Regex(@"^([aeiou]([^0-9|@|!|#|$|%|^|&|*|(|)|-|=|+|_|[|\]|\|\||,|<|.|>|/|\|?]))"); // Regex entry = new Regex(@"^([aeiouAEIOU])");
